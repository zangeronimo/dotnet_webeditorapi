...
public class DeleteUserUC(IUserRepository userRepository, IMapper mapper) : IUseCase<DeleteUserRequest, UserDto>
{
    private readonly IUserRepository _userRepository = userRepository;

    private readonly IMapper _mapper = mapper;

    public async Task<UserDto> ExecuteAsync(DeleteUserRequest request)
    {
        User? user = await _userRepository.GetByIdAsync(request.UserId, request.Context.CompanyId);
        if (user == null)
