...
public class CreateUserUC(IUserRepository userRepository, IPasswordProvider passwordProvider, IMapper mapper) : IUseCase<CreateUserModel, UserDto>
{
    ...
    public async Task<UserDto> ExecuteAsync(CreateUserModel request)
    {
        User? user = await _userRepository.GetByEmailAsync(request.Email);
        if (user != null)
            throw new ApiBadRequestException("Usuário já cadastrado com esse e-mail");
        User newUser = new User(request.Name, Email.Create(request.Email), Password.Create(request.Password, _passwordProvider), request.CompanyId);
